...
function parseRSS(xmlText, sourceName) {
  ...
  return [...xml.querySelectorAll("item")].map((item) => {
    ...
    const dateMatch = desc.match(/(?:last\s*date|closing|apply\s*by)[:\s]*(\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4})/i)
...
function getDept(t) {
  const map = ["SSC","UPSC","RRB","Railway","IBPS","SBI","Army","Navy","Air Force",
               "Police","NHM","DRDO","ISRO","High Court","Bank","Post Office","CRPF","BSF","CISF","NTA"];
...
function saveCache(jobs) {
  try { localStorage.setItem(CACHE_KEY, JSON.stringify({ jobs, ts: Date.now() })); } catch (_) {}
}
...
function renderStrip(jobs) {
  ...
  el.innerHTML = jobs.slice(0, 8).map(job => `
    <div onclick='openJob(${JSON.stringify(JSON.stringify(job))})'
      class="min-w-[200px] bg-surface-container-lowest p-4 rounded-xl shadow-sm border
             border-outline-variant/10 flex flex-col justify-between cursor-pointer
             hover:shadow-md transition-shadow">
      <div>
        <div class="flex justify-between items-start mb-2">
          <span class="bg-secondary-container text-on-secondary-container text-[10px]
                       font-black px-2 py-0.5 rounded-full uppercase">New</span>
          <span class="material-symbols-outlined text-outline text-lg">bookmark</span>
        </div>
        <h3 class="font-bold text-sm leading-tight text-on-surface mb-1">${job.title}</h3>
...
function renderFeed(jobs) {
  ...
  el.innerHTML = jobs.slice(0, 10).map(job => `
    <div class="bg-surface-container-lowest rounded-[24px] overflow-hidden
                shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-xl transition-all mb-8">
      <div class="p-8">
        <div class="flex items-start justify-between mb-4">
          <span class="bg-secondary-container text-on-secondary-container text-[12px]
                       font-black px-3 py-1 rounded-full uppercase">${job.source}</span>
        ...
        <button onclick='openJob(${JSON.stringify(JSON.stringify(job))})'
          class="w-full py-5 bg-gradient-to-r from-secondary to-on-secondary-container
                 text-white rounded-[20px] font-bold text-lg shadow-xl active:scale-95 transition-transform">
          Apply Now
        </button>
      </div>
    </div>
...
function showSkeletons() {
  ...
  if (feed) feed.innerHTML = Array(3).fill(`
    <div class="bg-surface-container-lowest rounded-[24px] p-8 animate-pulse flex flex-col gap-4 mb-8">
